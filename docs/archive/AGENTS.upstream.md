...
* 提交信息使用中文。代码、标识符、注释、规格与 ADR 仍用英文。
...
* §1 改动前阅读相关 spec / ADR。
* §2 默认保留既有行为；确需破坏性变更时必须记录影响与迁移路径。
* §3 架构边界；修改冻结架构、公共接口、数据所有权或安全边界需要 ADR。
* §9 数据库与持久化改动的迁移与升级兼容性。
* §10 安全、最小权限与错误处理；不削弱权限、沙箱、密钥边界。
* §11 可观察行为变化同步 spec；ADR 状态同步。
* §14 的最小集：只运行与被改动源码树相称的相关检查。
* §16 永不隐藏测试失败。
* §18 提交与 diff 卫生。
* §23 如实交接：改了什么、验证了什么、没跑什么、剩余风险。

### 0.3 降级

* §4 分支/worktree 隔离：不强制。除非所有者要求，不新建分支或 worktree，
  直接在 `main` 上工作；永不开发本任务不拥有的分支。
* §15 集成后 E2E：降级为"改动相关的最小验证"。全量 E2E 只在所有者明确
  要求时运行；未跑的套件一律记为 NOT RUN。
* §22 完成定义：分支/worktree、PR 门禁、集成后 E2E 三项不适用；以"逻辑
  改动已提交 + 验证状态如实上报"为准。

### 0.4 不再要求

* §12 / §13 GitHub issue 与 PR 接洽：仅当所有者提供上游链接作参考时适用。
* §19 远端发布与 PR 流程：远端交付 = 仅在所有者明确要求时把本 fork 的
  分支推送到 `origin`；不要求 PR，禁止推送上游。
* §20 集成与清理中的 PR 检查项。
* §21 中的发布、打包、签名与版本面资格认定，除非所有者要求出包。
* `docs/zh-CN/` 文档镜像与 `pnpm docs:check` 的 locale 一致性维护。
* 版本面门禁脚本（如 `scripts/check-release-docs.mjs`）不作为每次改动的
  前置条件，只在所有者要求打标签时考虑。

本文件其余条款（正确性、用户数据安全、安全、架构边界、改动粒度、提交
卫生）仍然完全生效。
...
* `docs/README.md` — 文档地图（活跃文档只有三件套）
* `docs/DECISIONS.md` — fork 本地决策 `F###`，改边界前先确认有没有既有决定
* `docs/archive/` — 上游继承的 spec / ADR / 交付文档，只作背景参考

本 fork 中，§3 / §11 / §22 要求的 "ADR" 等价物是 `docs/DECISIONS.md` 里新增一条
`F###`。除非任务明确要求，不要按 `docs/archive/` 里的发布、打包、PR、E2E 门禁流程
执行；口径以 §0 的单人 fork 精简流程为准。

归档文档保持英文；`docs/` 活跃三件套与提交信息使用中文。
...
`docs/archive/spec/06-delivery/04-e2e-test-plan.md`
