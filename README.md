...
简单串行任务由主 Agent 直接完成。仅在用户要求，或并行推进、上下文隔离、独立判断的具体收益超过交接与验收成本时委派；判断基于当前任务的实际收益。

1. **按实际收益选择委派**
   - 小范围检索和微小编辑直接完成，避免重复工作；用户明确要求派发时遵循指定范围与数量。
   - 探索、分析、验证可在能独立推进时委派；需要第二视角时可安排独立评审。
   - 主 Agent 负责整合与验收，用户决策由主 Agent 沟通；不自动套用探索、实施、测试、评审的固定流水线。
   - 派发后继续推进有用的独立工作，需要结果时使用 `TaskWait` 等待。
2. **并行只做独立的事，依赖必须等**
...
- **内置子 Agent 分工**：`worker` 负责独立的功能、模块与重构；`fixer` 聚焦缺陷和回归修复，另有探索、审查、测试及界面设计角色。
- **流式聊天展示**：主 Agent 输出时显示状态提示；完整思考使用独立滚动容器，高度上限为 `min(320px, 40vh)`，向上阅读暂停跟随，回到底部恢复，结束后保留自动收起行为。
- **窗口控制按钮**：最小化、最大化/还原、关闭采用统一的 32×32 方形按钮与细线图标；窗口按钮位于应用外层，右侧工作面板打开或最大化时保持可见。
- **插件多窗口接口**：`pluginBridge.widget.invoke` 提供窗口状态、位置与尺寸、鼠标穿透、置顶及同插件窗口开关；窗口入口由宿主解析，按插件隔离，关闭插件时同步清理窗口与未完成的打开请求。接口说明见 `apps/desktop/resources/skills/plugin-development.md`。
