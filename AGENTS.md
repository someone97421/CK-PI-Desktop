...
- 已吸收补丁与暂缓项记录在 `docs/UPSTREAM-SYNC.md`；逐项适配，不将上游整文件覆盖本 fork。
- 保留模型绑定、Google fetch 兼容、立即发送引导当前轮及子任务唤醒、分区字体/字重、浅深配色、模型配置导入导出、技能扩展路径和内置终端修复。
- 保留引用、批注和侧边对话；上游删除这些功能的补丁及依赖该删除的语言键清理，不自动同步。
- 扩展路径中的 linked 技能只读，移动功能须在界面与 host 两端拒绝这些技能。
- 上游 schema 17/18 的插件提供商归属和消息优先队列需要单独决定，不能因同步升级共用数据库。内置子智能体开关使用独立 `agent-capabilities/subagent-builtins.json`，不迁移数据库。
