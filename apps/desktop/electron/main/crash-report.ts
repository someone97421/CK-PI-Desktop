...
export function reportPreviousCrashDumps(input: {
  ...
  try {
    const markerDir = input.diagnosticsDir ?? input.dataDir ?? "";
    const crashDumpMarker = readCrashDumpMarker(markerDir)?.newestMtimeMs ?? null;
    ...
    if (
      ...
      writeCrashDumpMarker(markerDir, crashDumps.newestMtimeMs);
    }
  } catch (error) {
