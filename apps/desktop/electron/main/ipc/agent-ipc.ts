...
export function registerAgentIpc({
  ...
  handle(IPC.invoke.subagentStop, async (req: { sessionId: string; delegationId: string }) => {
    if (!sidecar) throw new Error("sidecar unavailable");
    ...
    return sidecar.call("agent.subagentStop", { sessionId: req.sessionId, delegationId: req.delegationId });
  });
