...
export function createRemoteBackend(options: RemoteBackendOptions): RemoteBackend {
  ...
  const invoke = async (channel: string, args: readonly unknown[]): Promise<unknown> => {
    switch (channel) {
      ...
      case IPC.invoke.agentQueueSteer:
        // The durable "send now" transfer rides the same local sidecar steer
        // path; a remote host serves neither, so both fail closed with the
        // same capability error instead of falling through to the local
        // handler for a session the local runtime does not own.
        throw capabilityUnavailable("steering is not available on a remote host");
