 ...
 * observe without Electron (local-only reporter, dumps under diagnosticsDir, start
 * before ready).
 */
...
test("a scan that is not a missing directory is reported, and never thrown", () => {
  ...
  const spy = loggerSpy();
  reportPreviousCrashDumps({
    diagnosticsDir: join(parentDir, "not-a-crash-data"),
    crashDumpsDirectory: notADir,
    logger: spy.logger,
  });
...
test("startup starts Crashpad local-only in the independent diagnostics directory before ready", () => {
  assert.match(startupSource, /crashReporter\.start\(\{\s*uploadToServer: false,/);
  assert.match(startupSource, /productName: APP_NAME/);
  assert.match(
    startupSource,
    /app\.setPath\(\s*"crashDumps",\s*ensureCrashDumpsDirectory\(diagnosticsDir\)\s*\)/,
  );
  ...
  assert.match(startupSource, /const diagnosticsDir = join\(app\.getPath\("userData"\), "diagnostics"\)/);
  const lockAt = startupSource.indexOf("if (!deps.hasSingleInstanceLock) return;");
  assert.ok(lockAt > 0 && setAt > lockAt, "only the lock holder initializes crash reporting");
  assert.ok(
...
test("startup reports new crash dumps after the single-instance lock", () => {
  assert.match(
    startupSource,
    /reportPreviousCrashDumps\(\{\s*diagnosticsDir,\s*crashDumpsDirectory:\s*app\.getPath\(\s*"crashDumps"\s*\),\s*logger,/,
  );
