...
test("updater gates delivery mode by platform and delivery policy", () => {
  ...
  assert.match(
    updaterSource,
    /github\.com\/\$\{APP_REPOSITORY\}\/releases/,
...
test("packaging publishes an electron-updater feed for GitHub Releases", () => {
  ...
  assert.equal(pkg.build.publish[0].owner, "someone97421");
  assert.equal(pkg.build.publish[0].repo, "CK-PI-Desktop");
  ...
  for (const script of ["dist", "dist:mac", "dist:win", "dist:linux"]) {
    assert.equal(pkg.scripts[script], `node ../../scripts/build.mjs ${script}`);
  }
  assert.equal(pkg.build.linux.executableName, "this-is-a-agent");
  ...
  assert.equal(pkg.build.deb.packageName, "this-is-a-agent");
  assert.equal(pkg.build.rpm.packageName, "this-is-a-agent");
  assert.ok(!pkg.build.deb.artifactName.includes("${name}"), "deb artifactName");
  assert.equal(
    pkg.build.rpm.artifactName,
    `this-is-a-agent-${pkg.buildVersionLabel}-` + '${arch}.${ext}',
    "rpm artifactName",
  );
  assert.deepEqual(
    pkg.build.rpm.fpm,
  ...
  assert.equal(pkg.build.nsis.artifactName, `this-is-a-agent-Setup-${pkg.buildVersionLabel}-` + '${arch}.${ext}');
