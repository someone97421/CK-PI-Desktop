...
test("旧配置的手动上下文值在保存和重新读取后仍然保留", async () => {
  const runtime = await fixtureRuntime();
  const legacy = { id: "terra", contextWindow: 400_000, maxTokens: 8_192, thinkingLevels: ["off"] };
  const first = runtime.enrichProvider(providerFor(legacy)).models[0];
  assert.notEqual(first.contextWindowSource, "catalog");
  const saved = JSON.parse(JSON.stringify(first));
  assert.equal(enrichedContextWindow(runtime, saved), 400_000);
});
