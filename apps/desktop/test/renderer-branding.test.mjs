...
test("renderer surfaces the 这是一个助手 brand instead of the Codex shell brand", () => {
  assert.match(english, /shellName:\s*"this-is-a-agent"/);
  assert.match(chinese, /shellName:\s*"这是一个助手"/);
  assert.match(english, /placeholder:\s*"Ask this-is-a-agent to help with anything"/);
  assert.match(chinese, /placeholder:\s*"让这是一个助手帮你做任何事"/);
