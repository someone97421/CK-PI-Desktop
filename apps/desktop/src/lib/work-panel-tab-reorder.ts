...
export function workPanelTabReorderShouldArm(dx: number, dy: number, thresholdPx = WORK_PANEL_TAB_REORDER_ARM_PX): boolean {
  return dx * dx + dy * dy > thresholdPx * thresholdPx;
}

export function workPanelTabReorderInsertAfter(clientX: number, targetLeft: number, targetWidth: number): boolean {
...
export function workPanelTabReorderScrollDelta(
  ...
  if (leftProgress > 0) return -Math.ceil(Math.min(1, leftProgress) * maxScrollPx);
  const rightProgress = (clientX - (stripRight - edgePx)) / edgePx;
  if (rightProgress > 0) return Math.ceil(Math.min(1, rightProgress) * maxScrollPx);
  return 0;
}
