 ...
 * 避免 sanitizer 和 ReactMarkdown 的 URL 过滤器将 href/src 清空。
 * 在 rehypeRaw 之后、sanitize 之前执行，覆盖链接和图片；
...
export function rehypeWindowsFileLinks() {
  return (tree: FileLinkHastNode) => {
    function walk(node: FileLinkHastNode) {
      ...
      if (node.type === "element" && key && node.properties && typeof value === "string") {
        const path = value.trim();
        if (/^\/?[A-Za-z]:(?:[/\\]|%5c)/i.test(path)) {
          node.properties[key] = `/${path.replaceAll("\\", "/").replace(/^\//, "")}`;
        }
      }
