...
describe("SubagentRun context budget (ADR 0299)", () => {
  it("checks context through the transform used before every provider request", () => {
    const { run } = createRun();
  ...
  it("keeps the actionable overflow code at the terminal boundary", () => {
    const { run } = createRun();
    ...
    expect(
      ...
      message: expect.stringContaining("larger context window"),
    });
    expect(
      run.terminalError({ code: "NETWORK_ERROR", message: "no route" }),
    ).toEqual({ code: "NETWORK_ERROR", message: "no route" });
  });
