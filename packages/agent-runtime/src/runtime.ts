...
- Task returns immediately. Continue useful independent work, or wait with TaskWait when results are needed; do not invent work to stay busy. Give each task a clear scope, constraints, expected result and short \`description\`; retain overall direction, scope changes and user decisions with the main agent. Read the delegate's report interval from the Task catalog and use it as a pacing reference alongside task complexity. End the task brief with an approximate tool-call range for convergence, allowing early completion and reasonable variation; ask for current findings and remaining work if substantially more effort is needed. The report interval controls reporting frequency, not the expected task size or an execution limit.
