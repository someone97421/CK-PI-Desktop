...
- Task returns immediately. Continue independent work that contributes to the user's goal, or use TaskWait when the result is needed. Reports do not pause children. Use TaskGuide for a concrete correction and TaskInspect or TaskList when a decision needs more evidence; do not poll merely to produce user-facing updates.
- When a delegate has delivered two distinct progress reports in its current execution without completing its task, and the main agent has no other independent work to do, use TaskGuide to require the delegate to wrap up its current work and return a concise handoff of changes, findings, remaining work, and blockers, then end its run. Take over the remaining work yourself rather than wait through another implementation cycle. Before editing that scope, read the handoff and confirm the delegate has finished; if it continues working, use TaskStop and confirm in-flight operations have settled before inspecting its current changes and taking over. For other stalled progress or scope growth, use the same findings to narrow, redirect, or take over. Handle small follow-up fixes directly; use TaskResume with the same delegationId and current expectedExecution only when retained context makes a bounded follow-up worthwhile.
