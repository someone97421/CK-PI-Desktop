export const en = {
  ...
  tray: {
    open: "Open 这是一个助手",
    quit: "Quit 这是一个助手",
    askTitle: "Keep 这是一个助手 running in the background?",
    askBody:
      "When you close the window, 这是一个助手 can keep running in the system tray so nothing is lost. You can change this any time in Settings.",
    closeToTray: "Close to tray",
    confirmQuitTitle: "Quit 这是一个助手?",
    confirmQuitBody:
  ...
  chat: {
    ...
    placeholder: "Ask 这是一个助手 to help with anything",
...
sklm: {
    ...
    piSyncDesc:
      "Manually copy provider definitions, API keys, and the default model between 这是一个助手 and the system pi CLI (~/.pi/agent). Subscription logins are never copied.",
    ...
    defaultPermissionsDesc: "By default, 这是一个助手 can read and edit files in your project. It will ask before doing anything riskier.",
    permissionMode: "Permission mode",
    ...
    autoReviewDesc: "这是一个助手 can read and edit files in your project, and will automatically decide on extra access requests. Auto-review can make mistakes.",
    learnMore: "Learn more",
    autoReviewLearnMoreAfter: " about high-risk information.",
    fullAccess: "Full access permissions",
    fullAccessDesc: "With full access, 这是一个助手 can edit files anywhere on your computer and run network commands without asking. This raises the risk of data loss or unexpected changes.",
    fullAccessLearnMoreAfter: " about elevated risks.",
    showMenuBar: "Show in menu bar",
    showMenuBarDesc: "Keep 这是一个助手 in the macOS menu bar when the main window is closed.",
    ...
    closeBehaviorTrayDesc: "Keep 这是一个助手 running in the system tray.",
    closeBehaviorQuit: "Quit app",
    closeBehaviorQuitDesc: "Exit 这是一个助手 completely.",
...
importConfirm: "Imported extensions run inside the agent process with the same access as the agent's own tools. Declared skills can add instructions to the agent, and declared dependencies are installed with npm (install scripts disabled). Continue?",
    ...
    permissionHelp: {
      ...
      "desktop.control":
        "Lets the plugin invoke the reviewed 这是一个助手 control catalog. Destructive operations still require confirm=true; the MCP bearer token is never exposed.",
  ...
  status: {
    ...
    dbSchemaTooNew:
      "This 这是一个助手 is older than your local data (data schema {{found}}, this build supports {{supported}}). Install the newer 这是一个助手 that last opened this data, or a later version.",
    archMismatch:
