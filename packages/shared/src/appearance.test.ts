...
describe("optional appearance settings", () => {
  ...
  it("validates known colors and fonts while allowing future keys", () => {
    ...
    for (const value of [null, [], { light: [] }, { dark: { code: { weight: 1001 } } }, { light: { ui: { family: "x; color: red" } } }, { dark: { accent: "red" } }]) {
  ...
  it("normalizes corrupted optional fields without discarding future settings", () => {
    ...
    expect(normalizeAppearance({ future: true, light: { accent: "#abc", background: "bad", ui: { family: "Inter", weight: 1001 }, future: 7 }, dark: null }))
  ...
  it("preserves the complete CSS weight range including variable fractional values", () => {
    for (const weight of [1, 50, 100, 350.5, 900, 950, 1000]) {
      const appearance = { dark: { code: { weight } } };
      expect(isAppearanceSettings(appearance)).toBe(true);
      expect(normalizeAppearance(appearance)).toEqual(appearance);
    }
    for (const weight of [0, -1, 1000.1, NaN, Infinity, "400"]) {
      expect(isAppearanceSettings({ light: { ui: { weight } } })).toBe(false);
    }
  });
});
