...
describe("RuntimeService prompt lifecycle", () => {
  ...
  it("refuses a per-turn ceiling that differs from the session's stored mode before any turn state", async () => {
    // The sidecar's `agent.prompt` override is a stub and host-core has no
    // per-turn parameter, so a narrowed ceiling would silently run at the
    // session's stored `auto` mode. The gate must fire before beginTurn and
    // appendMessage: no turn record, no transcript row, no runtime prompt.
    const { host, sidecar, service } = build();
    host.session.permissionMode = "auto";
    await expect(
      service.prompt({ sessionId: "s1", content: "hello", effectivePermissionMode: "ask", principal: owner }),
    ).rejects.toMatchObject({ errorCode: "FORBIDDEN" });
    expect(host.calls.some((call) => call.method === "session.beginTurn")).toBe(false);
    expect(host.calls.some((call) => call.method === "session.appendMessage")).toBe(false);
    expect(sidecar.calls.some((call) => call.method === "agent.prompt")).toBe(false);
  });

  it("normalizes a non-canonical stored mode to ask so a matching ask ceiling starts", async () => {
    const { host, sidecar, service } = build();
    host.session.permissionMode = "inherit";
    await service.prompt({ sessionId: "s1", content: "hello", effectivePermissionMode: "ask", principal: owner });
    expect(sidecar.calls.some((call) => call.method === "agent.prompt")).toBe(true);
  });
