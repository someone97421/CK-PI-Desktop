...
/** The per-turn modes the runtime understands; anything else falls to `ask`. */
const PERMISSION_MODES = new Set(["ask", "accept-edits", "auto"]);

function normalizePermissionMode(value: unknown): string {
  return typeof value === "string" && PERMISSION_MODES.has(value) ? value : "ask";
}
...
export class RuntimeService implements RuntimePort {
  ...
  /**
   * Per-turn permission ceiling (spec §7.3). The sidecar's `agent.prompt`
   * override is a stub and host-core `session.beginTurn` has no per-turn
   * parameter, so a ceiling that differs from the session's stored mode would
   * silently run at the stored mode. Fail closed before any turn state is
   * created or transcript written; only a matching mode may start a turn.
   */
  private assertPerTurnPermissionMode(
    session: Record<string, unknown> | null | undefined,
    effective: string,
  ): void {
    const stored = normalizePermissionMode(
      session && typeof session === "object" ? (session as { permissionMode?: unknown }).permissionMode : undefined,
    );
    if (normalizePermissionMode(effective) !== stored) {
      throw typedError(
        "the headless runtime cannot apply a per-turn permission ceiling yet",
  ...
  private async startTurn(sessionId: string, request: TurnStartRequest): Promise<{ turnId: string }> {
    ...
    // Before beginTurn/appendMessage: a refused ceiling must leave no turn
    // record and no transcript row behind.
    this.assertPerTurnPermissionMode(session, request.effectivePermissionMode);
    ...
    try {
      result = await sidecar.call<{ accepted: boolean; turnId: string }>("agent.prompt", {
        ...
        // this call could ever run at the session's stored mode.
      });
    } catch (error) {
      await this.finishTurn(sessionId, "error", errorCodeOf(error), { turnId });
      throw error;
