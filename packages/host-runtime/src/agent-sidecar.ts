...
/**
 * Handler an embedding host registers for a sidecar proxy method outside the
 * shared allowlist (e.g. host-specific persistence). Registering one extends
 * the proxy gate for exactly that method; anything else still fails closed.
 */
export type SidecarProxyMethodHandler = (
...
export class AgentSidecar {
  ...
  // Embedding-host handlers for proxy methods outside the shared allowlist
  // (e.g. host-specific persistence). Registered methods extend the gate;
  // everything else still fails closed.
  private proxyMethods = new Map<string, SidecarProxyMethodHandler>();
  private trustedExtensionBridge: TrustedExtensionSidecarBridge | null = null;

  /**
   * Register a handler for one sidecar proxy method outside the shared
   * allowlist. Registering extends the gate for exactly that method; a
   * registered method whose handler is missing still fails closed.
   */
  setProxyMethodHandler(method: string, handler: SidecarProxyMethodHandler): void {
    const name = method.trim();
    if (!name) return;
    this.proxyMethods.set(name, handler);
  }
  ...
  private async onLine(line: string) {
    ...
    if (msg.method === "host.proxy" && msg.id !== undefined) {
      try {
        ...
        const proxyHandler = this.proxyMethods.get(method);
        if (!HOST_PROXY_ALLOWED.has(method) && !proxyHandler) {
          throw Object.assign(
        ...
        if (proxyHandler) {
          // A registered embedding-host method answers before the shared
          // allowlist path; the handler owns all session binding checks.
          const result = await proxyHandler(params);
          this.writeToChild(
  ...
  /** True once the transport is closed or the sidecar is disposed. */
  isClosed(): boolean {
