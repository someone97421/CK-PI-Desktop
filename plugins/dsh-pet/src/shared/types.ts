...
/** config.jsonc 的 characters.<id> 段：非默认角色（当前只有精灵图集角色）的动作池覆盖。
 *  字段与条目级同名同形；未写的字段回落条目级（flattenConfigPets 逐字段判断）。 */
export interface CharacterProfile {
  /** 角色显示名（设置页下拉用；缺省用 id） */
  label?: string;
  /** 图集文件（相对 assets/；仅图集角色有） */
  sheet?: string;
...
export interface Pet {
  ...
  workStatusTexts?: string[][];
  /** 角色 id（可选，省略 = 'maid' 女仆）：决定用哪套素材与动作池。
   *  'maid' = assets/webm 的透明 webm；其余 id 见 config.jsonc 的 characters 段（图集角色）。 */
  character?: string;
  /** 图集文件（相对 assets/，由 flattenConfigPets 从角色档吹入；仅图集角色有） */
...
export interface ClientConfig {
  ...
  eventsRefreshSec: Record<string, number>;
  /** 角色档案（config.jsonc 的 characters 段，逐宠物按 id 取用；由 flatten 吹进实例） */
