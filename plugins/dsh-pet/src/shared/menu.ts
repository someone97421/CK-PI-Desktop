...
export interface MenuLeaf {
  ...
  anim?: string;
  /** 自定义动作。移植后：whisper=立刻碎碎念一句；home=回到初始位置；
   *  close-pet=收起这只宠物；toggle-pin=切换窗口置顶；hide-all=收起全部宠物；
   *  open-settings=打开桌面宠物设置页。手动触发均不受 whisperEnabled 影响。
   *  （原版的 open-site / show-balance / chat 是 DSH 专属能力，没有对应实现，不在此列。） */
  action?: 'whisper' | 'home' | 'close-pet' | 'toggle-pin' | 'hide-all' | 'open-settings';
}
