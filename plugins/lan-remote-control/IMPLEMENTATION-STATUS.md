...
| 审批/提问/计划 | web/app.js、host-adapter.cjs | 决议入口、多选/其他/跳过、Plan/Goal 提案；**按用户决议手机提交直接生效，移除电脑二次确认**（由宿主 plugin-runtime 与 adapter 直接执行） |
| 模型及会话选项 | models.list、models.configure | 模型/思考级别、工作模式、权限模式；**按用户决议手机登录后可直接更改，直接生效且无需电脑确认** |
...
1. **独立手机审批与模型配置**：已按密码登录后直接操作实现，需更新主程序和插件；尚未实机验证。
2. **侧边对话跨端关联**：桌面关系为 renderer-only。手机当前只保存自身父子映射，尚未与桌面工作面板共享。
...
6. **界面收口**：尚使用浏览器 prompt/confirm 承载部分编辑/批注；桌面 renderer 专属命令在远程仍未实现并保持禁用；能力禁用没有覆盖全部入口；搜索只覆盖已加载的会话页；大段 Markdown/高频工具事件性能未验证；PDF 未提供内嵌内容预览。
7. **主应用类型与运行集成**：只做改动 TS 语法检查和 SDK 类型检查，未做完整宿主类型检查。安装、启停/崩溃、审批竞争、真实手机/软键盘均未运行。

## 移动面板与独立远控修复

- 弹层不再因失焦、原生选择器或切换后台而移除，统一保留关闭按钮、遮罩和 Esc 退出。
- 图片上传补齐空 MIME 类型，上传及失败状态逐文件显示，支持重试和重复选择同一文件；请求超时覆盖响应正文。
- 局域网插件的全部宿主操作采用密码登录授权，配置与审批在手机直接提交；清理原生确认提示与能力标记。
- 本次未运行测试、构建或手机实测。
